...
int main() {
    ...
    std::printf("  Taylor roundtrip check: %s\n\n",
        check_taylor_roundtrip_precision(p) ? "PASS" : "FAIL (precision loss)");
    ...
    std::printf("  Taylor roundtrip: %s\n\n",
        check_taylor_roundtrip_precision(big) ? "PASS" : "FAIL (expected — coefficient too large)");

    auto p_at_3 = p.eval(3);
    auto ff_at_3 = ff.eval(3);
    auto t_at_3 = taylor.eval(3);
